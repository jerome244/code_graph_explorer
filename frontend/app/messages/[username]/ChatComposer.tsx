...
export default function ChatComposer({
  ...
  function insertDoc() {
    ...
    let msg = '';
    if (trimmedUrl) {
      msg = `Please join this doc: ${trimmedUrl}`;
    } else if (trimmedPid) {
      msg = `Please join my project: /graph?projectId=${encodeURIComponent(trimmedPid)}`;
    }
    if (msg) {
      insert((body ? '\n' : '') + msg);
      setShowDoc(false);
  ...
  return (
    <form action={action} style={{ display: 'grid', gap: 8 }}>
      <input type="hidden" name="to" value={toUsername} />

      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        <button
          type="button"
          onClick={() => setShowEmoji(v => !v)}
          style={toolBtn as any}
          title="Insert emoji"
        >
          😊 Emoji
        </button>
