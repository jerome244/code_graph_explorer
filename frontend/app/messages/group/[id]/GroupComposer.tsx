...
export default function GroupComposer({ groupId }: { groupId: string }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await fetch('/api/messages/groups/send', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ group_id: groupId, body: text }),
      });

      if (res.status === 403) {
        alert((await res.text().catch(() => '')) || 'Messaging is blocked in this group.');
        return;
      }
      ...
      if (res.ok) {
        ...
        // notify listeners (if you add any) + refresh the page data
        window.dispatchEvent(new CustomEvent('group:new', { detail: { message: created, groupId } }));
        router.refresh();
      } else {
    ...
    <form onSubmit={onSubmit} style={{ display: 'grid', gap: 8 }}>
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
        <button type="submit" disabled={!body.trim() || busy}
          style={{ padding: '10px 14px', borderRadius: 8, border: '1px solid #2563eb', background: '#2563eb', color: '#fff', fontWeight: 600, cursor: 'pointer', opacity: (!body.trim() || busy) ? 0.6 : 1 }}>
          {busy ? 'Sending…' : 'Send'}
        </button>
      </div>
    </form>
